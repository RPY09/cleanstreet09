...
const AdminReports = () => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(12);
      doc.text("Community Issues Resolution System", 105, 30, {
        align: "center",
      });
      ...
      const performanceData = [
        [
          ...
          reports.systemMetrics.avgResponseTime || "N/A",
        ],
      ...
      const priorityTotal = priorityEntries.reduce(
        (sum, [, count]) => sum + (Number(count) || 0),
        0
      );
      ...
      const priorityData = priorityEntries.map(([priority, count]) => [
        ...
        count,
        priorityTotal > 0
          ? `${((count / priorityTotal) * 100).toFixed(1)}%`
          : "0%",
      ]);
      ...
      const fileName = `admin-report-${getTimeRangeLabel(timeRange)
        .toLowerCase()
